...
def userrequests():
    ...
    query=db.engine.execute(f"SELECT * FROM `postrequest` WHERE flatno='{em}'")
...
@login_required
def requests4():
    query=db.engine.execute(f"SELECT * FROM `requests` WHERE `flatno`='{current_user.flatno}'")
    return render_template('requests.html',query=query)    
@app.route('/profile')
@login_required
def profile():
...
@app.route('/requests3')
def request3():
    query=db.engine.execute(f"SELECT * FROM `requests` ORDER BY `date` DESC")
